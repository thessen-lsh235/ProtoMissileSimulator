...
class Target : public Entity {
  private:
    std::string name; // 표적 이름
    double x, y;
    double speed;  // 속도 (단위: 거리/초)
    double degree; // 각도 (도 단위)

  public:
    Target(const std::string &name, double x, double y, double speed, double degree)
        : name(name), x(x), y(y), speed(speed), degree(degree) {}
    ...
    void updatePosition(double delta_time) override {
        ...
        // 맵 경계 초과 여부 확인
        checkBounds(name, x, y);

        fmt::print("[{}] Target updated - x: {:.2f}, y: {:.2f}, degree: {:.2f}, speed: {:.2f}\n", name, x, y, degree, speed);
    }
