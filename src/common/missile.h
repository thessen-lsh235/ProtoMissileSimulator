...
struct MissileInfo : public Serializable<MissileInfo>
{
	...
	{
		std::vector<uint8_t> buffer(1 + sizeof(missile_id) + sizeof(LS_pos_x) + sizeof(LS_pos_y) + sizeof(speed) + sizeof(degree));
		buffer[0] = static_cast<uint8_t>(DataType::Missile); // MissileInfo 타입 식별자
		size_t offset = 1;
		std::memcpy(buffer.data() + offset, &missile_id, sizeof(missile_id));
		offset += sizeof(missile_id);
	...
	{
		size_t offset = 0;
		std::memcpy(&missile_id, buffer.data() + offset, sizeof(missile_id));
		offset += sizeof(missile_id);
