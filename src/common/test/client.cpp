...
int main() {
    ...
    while (true) {
        ...
        if (message_type == static_cast<int>(DataType::Missile)) {
            ...
            serialized_data = missile.serializeImpl();
            ...
            serialized_data = target.serializeImpl();
        } else {
        ...
        // 데이터 전송
        sendto(socket_fd, serialized_data.data(), serialized_data.size(), 0, (const struct sockaddr *)&server_addr, sizeof(server_addr));
